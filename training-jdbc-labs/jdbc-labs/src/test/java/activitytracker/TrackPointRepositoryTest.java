...
class TrackPointRepositoryTest {
    ...
    void getDataSourceTest() {
        assertEquals(mariaDbDataSource, trackPointRepository.getDataSource());
    }
    ...
    @Test
    void saveTrackPointsForActivityTest() {
        assertEquals(0, trackPointRepository.getAllTrackPoints().size());

        trackPoints = List.of(
            new TrackPoint(LocalDate.of(2020, 10, 20), 47.5, 19.05),
            new TrackPoint(LocalDate.of(2020, 10, 10), 47.5, 21.58)
        );
        trackPointRepository.saveTrackPointsForActivity(trackPoints, 5L);
        assertEquals(2, trackPointRepository.getAllTrackPoints().size());
        assertEquals(5L, trackPointRepository.getActivityIdByTrackPointId(1L));
        assertEquals(5L, trackPointRepository.getActivityIdByTrackPointId(2L));

        trackPoints = List.of(
            new TrackPoint(LocalDate.of(2020, 10, 20), 47.5, 19.05),
            new TrackPoint(LocalDate.of(2020, 10, 10), 47.5, 215.8)
        );
        trackPointRepository.saveTrackPointsForActivity(trackPoints, 3L);
        assertEquals(2, trackPointRepository.getAllTrackPoints().size());
    }
